...
function RepoIssue(props) {
  if (props.loading) {
    return <Loading />;
  }
  return (
    <Center>
      <Tabs mt="2em" align="center">
        <TabList mb=".5em">
          <Tab>Open</Tab>
          <Tab>Closed</Tab>
        </TabList>
        <TabPanels>
          <TabPanel>
            {props.data.repository.openIssues.edges.map((issue) => {
              return (
                <Link
                  to={`${props.name + "/" + issue.node.number}`}
                  key={issue.node.id}
                >
                  <Box
                    marginX="auto"
          ...
          <TabPanel>
            {props.data.repository.closedIssues.edges.map((issue) => {
              return (
                <Link
                  to={`${props.name + "/" + issue.node.number}`}
                  key={issue.node.id}
